...
export const getInternships = () => async (dispatch) => {
  try {
    ...
    if (res.data.status === "success") {
      console.log(res.data.data);
      dispatch(addInternships(res.data.data.data.internships));
    }
  } catch (err) {
    console.log("error", err.response.data.message);
...
export const patchInternships = (
  ...
  try {
    const res = await axios({
      method: "PATCH",
      url: `http://127.0.0.1:5000/api/v1/internship/${id}`,
      headers: {
        Authorization: `Bearer ${token}`,
      },
      data: {
...
export const deleteInternships = (id) => async (dispatch) => {
  try {
    await axios({
      method: "DELETE",
      url: `http://127.0.0.1:5000/api/v1/internship/${id}`,
